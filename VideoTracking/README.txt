...
Flags available to be set in the config file:
	...
	DEBUGGING - allow outputting more information during runtime for debugging purposed
...
User first is prompt to select the video file and then has to either:
	wait for the background subtraction to automatically perform the task, 
	or click on all of the fish in the frame and press 'n' to go to the next frame. 
